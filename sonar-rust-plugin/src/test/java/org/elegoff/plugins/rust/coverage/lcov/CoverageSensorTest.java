...
public class CoverageSensorTest  {
    ...
    public void init() throws FileNotFoundException {
        ...
        //inputFile("tests/file1.rs", InputFile.Type.TEST);
    }
    ...
    public void report_not_found() throws Exception {
        settings.setProperty(RustPlugin.LCOV_REPORT_PATHS, "/wrong/path/lcov_report.dat");
    ...
    public void test_coverage() {
        settings.setProperty(RustPlugin.LCOV_REPORT_PATHS, TWO_REPORTS);
        coverageSensor.execute(context);
    ...
    public void should_ignore_and_log_warning_for_invalid_line() {
        settings.setProperty(RustPlugin.LCOV_REPORT_PATHS, "reports/wrong_line_report.lcov");
        coverageSensor.execute(context);
        ...
        assertThat(logTester.logs()).contains("Problem during processing LCOV report: can't save DA data for line 3 of coverage report file (java.lang.IllegalArgumentException: Line with number 0 doesn't belong to file file1.rs).");
        assertThat(logTester.logs()).contains("Problem during processing LCOV report: can't save BRDA data for line 8 of coverage report file (java.lang.IllegalArgumentException: Line with number 102 doesn't belong to file file1.rs).");
    }

    @Test
    public void test_unresolved_path() {
        settings.setProperty(RustPlugin.LCOV_REPORT_PATHS, "reports/report_with_unresolved_path.lcov");
        coverageSensor.execute(context);
    ...
    public void should_log_warning_when_wrong_data() throws Exception {
        settings.setProperty(RustPlugin.LCOV_REPORT_PATHS, "reports/wrong_data_report.lcov");
        coverageSensor.execute(context);
    ...
    public void should_resolve_relative_path() throws Exception {
        settings.setProperty(RustPlugin.LCOV_REPORT_PATHS, "reports/report_relative_path.lcov");
    ...
    public void should_resolve_absolute_path() throws Exception {
        ...
        settings.setProperty(RustPlugin.LCOV_REPORT_PATHS, lcovFile.getAbsolutePath());
        inputFile("file1.rs", InputFile.Type.MAIN);
        inputFile("file2.rs", InputFile.Type.MAIN);
        coverageSensor.execute(context);
