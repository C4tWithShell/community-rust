...
public enum RustGrammar implements GrammarRuleKey {
    ...
    STRUCT_STRUCT,
    TUPLE_STRUCT,
    TUPLE_FIELDS, STRUCT_FIELD, TUPLE_FIELD,
    FUNCTION_PARAMETERS,
    ...
    INTEGER_SUFFIX, OCT_LITERAL,
    BIN_LITERAL,
    BIN_DIGIT, OCT_DIGIT,
    HEX_DIGIT,
    NON_ZERO_DEC_DIGIT, LITERALS,
    LIFETIMES, PUNCTUATION, DELIMITERS,
    NON_KEYWORD_IDENTIFIER,
    IDENTIFIER,
    RAW_IDENTIFIER, ASCII_FOR_STRING,
    QUOTE_ESCAPE,
    ASCII_ESCAPE, UNICODE_ESCAPE, RAW_STRING_CONTENT, FLOAT_EXPONENT, FLOAT_SUFFIX, RAW_BYTE_STRING_CONTENT;
    ...
    public static Grammar create() {
        ...
        b.rule(FILE_INPUT).is(WHITESPACE, EXPRESSION, b.endOfInput());
    ...
    public static void lexicaltoken(LexerlessGrammarBuilder b) {
        ...
        b.rule(PUNCTUATION).is(b.regexp(toRegexp(RustPunctuator.punctuatorValues())));
    ...
    private static void bytes(LexerlessGrammarBuilder b) {
        b.rule(BYTE_LITERAL).is("b'", b.firstOf(ASCII_FOR_CHAR , BYTE_ESCAPE), "'");
    ...
    private static void identifiers(LexerlessGrammarBuilder b) {
        b.rule(IDENTIFIER_OR_KEYWORD).is(b.regexp(toRegexp(RustKeyword.keywordValues())));
        b.rule(RAW_IDENTIFIER).is("r#",IDENTIFIER_OR_KEYWORD);//except Except crate, self, super, Self
        b.rule(NON_KEYWORD_IDENTIFIER).is(IDENTIFIER_OR_KEYWORD);//Except a strict or reserved keyword
        b.rule(IDENTIFIER).is(b.firstOf(NON_KEYWORD_IDENTIFIER, RAW_IDENTIFIER));
    }
    ...
    private static void integerliteral(LexerlessGrammarBuilder b) {
        b.rule(INTEGER_LITERAL).is(b.firstOf(DEC_LITERAL, BIN_LITERAL, OCT_LITERAL, HEX_LITERAL), b.optional(INTEGER_SUFFIX));
